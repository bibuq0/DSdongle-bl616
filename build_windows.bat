...
rem ---- auto-locate toolchain: try candidate paths in order when TOOLCHAIN_PATH is not set ----
if "%TOOLCHAIN_PATH%"=="" (
    if exist "%USERPROFILE%\Desktop\toolchain_gcc_t-head_windows\bin\riscv64-unknown-elf-gcc.exe" (
        set "TOOLCHAIN_PATH=%USERPROFILE%\Desktop\toolchain_gcc_t-head_windows"
    ) else if exist "%USERPROFILE%\Desktop\toolchain_gcc_t-head_windows-master\toolchain_gcc_t-head_windows-master\bin\riscv64-unknown-elf-gcc.exe" (
        set "TOOLCHAIN_PATH=%USERPROFILE%\Desktop\toolchain_gcc_t-head_windows-master\toolchain_gcc_t-head_windows-master"
    )
)
